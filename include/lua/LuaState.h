...
namespace LPCD
{
	...
	inline int LuaStateOldFunctionDispatcher(lua_State* L)
	{
		...
		numArgs = 20;
#if defined(_MSC_VER)
		LuaPlus::LuaStackObject* args = (LuaPlus::LuaStackObject*)_alloca(sizeof(LuaPlus::LuaStackObject) * (numArgs + 1));
#else
	...
	public:
		static inline int LuaStateOldMemberDispatcher(lua_State* L)
		{
			...
			Callee& callee = **(Callee**)buffer;
			Functor& func = *(Functor*)(buffer + sizeof(Callee*));
			LuaState* state = (LuaPlus::LuaState*)lua_getstateuserdata(L);
			int numArgs = lua_gettop(L);
			numArgs = 20;
#if defined(_MSC_VER)
			LuaPlus::LuaStackObject* args = (LuaPlus::LuaStackObject*)_alloca(sizeof(LuaPlus::LuaStackObject) * (numArgs + 1));
...
public:
	...
	LuaStackObject PushCClosure(const Callee& callee, int (Callee::*f)(LuaState*), int n)
	{
		...
		lua_pushcclosure(m_state, &LPCD::LuaStateMemberDispatcherHelper<Callee>::LuaStateMemberDispatcher, n + 1);
